...
void SYSCLOCK_Start(void) 
{
    ...
    SYSCLOCK_CLKEN |= SYSCLOCK_CLKEN_MASK;
	SYSCLOCK_CLKSTBY |= SYSCLOCK_CLKSTBY_MASK;
}
...
void SYSCLOCK_Stop(void) 
{
    ...
    SYSCLOCK_CLKEN &= (uint8)(~SYSCLOCK_CLKEN_MASK);
	SYSCLOCK_CLKSTBY &= (uint8)(~SYSCLOCK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: SYSCLOCK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((SYSCLOCK_CLKEN & SYSCLOCK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = SYSCLOCK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = SYSCLOCK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* SYSCLOCK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(SYSCLOCK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(SYSCLOCK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: SYSCLOCK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SYSCLOCK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = SYSCLOCK_GetSourceRegister();
    uint16 oldDivider = SYSCLOCK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = SYSCLOCK_CLKEN & SYSCLOCK_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((SYSCLOCK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: SYSCLOCK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(SYSCLOCK_DIV_PTR);
}


/*******************************************************************************
* Function Name: SYSCLOCK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    SYSCLOCK_MOD_SRC |= modeBitMask & (uint8)SYSCLOCK_MODE_MASK;
}


/*******************************************************************************
* Function Name: SYSCLOCK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    SYSCLOCK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(SYSCLOCK_MODE_MASK));
}


/*******************************************************************************
* Function Name: SYSCLOCK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return SYSCLOCK_MOD_SRC & (uint8)(SYSCLOCK_MODE_MASK);
}


/*******************************************************************************
* Function Name: SYSCLOCK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = SYSCLOCK_GetDividerRegister();
    uint8 oldSrc = SYSCLOCK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        SYSCLOCK_MOD_SRC |= CYCLK_SSS;
        SYSCLOCK_MOD_SRC =
            (SYSCLOCK_MOD_SRC & (uint8)(~SYSCLOCK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: SYSCLOCK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return SYSCLOCK_MOD_SRC & SYSCLOCK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: SYSCLOCK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    SYSCLOCK_PHASE = clkPhase & SYSCLOCK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: SYSCLOCK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return SYSCLOCK_PHASE & SYSCLOCK_PHASE_MASK;
}
