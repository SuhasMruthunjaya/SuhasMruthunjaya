...
#if (Number_of_ISRs > 1) && defined __INTC_NUMBER
    ramVectorTable[CY_INT_IRQ_BASE + (__INTC_NUMBER & CY_INT_NUMBER_MASK)] = ;
	//PF v2.5.3
    //NVIC_SET_PRI(__INTC_NUMBER, (ISR_MIN_PRI - 0));
    CyIntSetPriority(__INTC_NUMBER, ISR_SCALE(0));
    //\PF
	NVIC_INT_ENABLE(__INTC_NUMBER);
#elif Number_of_ISRs > 1
#error  The Interrupt  could not be found. Make sure the name in the OS config exactly matches the name of the interrupt in the interrupt tab.
#endif	/*  */
