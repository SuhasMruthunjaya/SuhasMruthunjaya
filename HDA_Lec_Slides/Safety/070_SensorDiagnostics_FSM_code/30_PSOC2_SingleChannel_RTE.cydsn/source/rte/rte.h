 ...
 **/
typedef void (*RTE_pRunnable)(void);
...
/**
 * Returns the ticktime for the given runnable
 * \param RTE_cyclicTable_t const *const table : [IN] table to be processed
 * \param uint16_t tableSize : [IN] table size
 * \return ticktime in ms, 0 in case runnable is not listed
 **/
uint16_t RTE_getTickTime(RTE_cyclicTable_t const *const table, uint16_t tableSize, RTE_pRunnable pRun);
