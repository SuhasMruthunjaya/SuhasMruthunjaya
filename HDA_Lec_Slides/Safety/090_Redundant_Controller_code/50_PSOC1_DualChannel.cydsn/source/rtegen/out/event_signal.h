...
#ifndef EVENT_SIGNAL_H_INCLUDED
#define EVENT_SIGNAL_H_INCLUDED
 ...
 *  I-Port
 */
typedef RC_t (*EVENT_driverIn_t)(EVENT_data_t * const);

/**
 *  Definition of function types for driver interfaces.
 *  O-Port
 */
typedef RC_t (*EVENT_driverOut_t)( const EVENT_data_t * const);
...
/**
 * Instances of the struct EVENT_cfg_t represent the signal object configuration.
 */
struct EVENT_cfg_s
{
    /**
     ...
     */
    EVENT_driverIn_t const inDriver;

    /**
     * Function pointer to the output driver table
     */
   EVENT_driverOut_t const outDriver;
...
typedef const struct EVENT_cfg_s EVENT_cfg_t;
...
{                                                                                                                      
     ...
     */
	const EVENT_cfg_t * const    pSigCfg;
 ...
 * @param const event_data_t * const value: Pointer to initial value of the application data
 * @return RC_t: standard return code, currently only RC_SUCCESS
 *
 **/
inline RC_t RTE_EVENT_init( EVENT_t * const signal, const EVENT_data_t * const value )
{                                                                                                                      
 ...
 *
 **/
inline RTE_signalStatus_t RTE_EVENT_getStatus( const EVENT_t * const signal )
{
 ...
 *
 */
inline RC_t RTE_EVENT_setStatus( EVENT_t * const signal, const RTE_signalStatus_t status )
{
 ...
 */
inline uint32_t RTE_EVENT_getAge( const EVENT_t * const signal )
{
   return signal->age;
}
 ...
 * @return void
 */
inline void RTE_EVENT_incAge(  EVENT_t * const signal , uint32_t increment)
{
	   //Hack....
 ...
 *
 **/
inline EVENT_data_t RTE_EVENT_get( const EVENT_t * const signal )
{                                                                                                                      
   return signal->value;                                                                                               
}                                                                                                                      
 ...
 *
 **/
inline RC_t RTE_EVENT_set(EVENT_t * const signal, const EVENT_data_t value  )
{
 ...
 *
 */
inline RC_t RTE_EVENT_pullPort( EVENT_t * const signal )
{                                                                                                                      
   RC_t ret = RC_SUCCESS;
   ...
   {
	  EVENT_data_t value = signal->value;
      ret = signal->pSigCfg->inDriver(&value);
                                                                                                                       
      if ( ret == RC_SUCCESS )                                                                                         
      {
    	  //Critical Section
    	  RTE_EVENT_set(signal, value);
      }
      else
      {
    	  //Set the status and fire error event if available
    	  RTE_EVENT_setStatus( signal, RTE_SIGNALSTATUS_INVALID );
 ...
 *
 **/
inline RC_t RTE_EVENT_pushPort( const EVENT_t * const signal )
{                                                                                                                      
   RC_t ret = RC_ERROR;
