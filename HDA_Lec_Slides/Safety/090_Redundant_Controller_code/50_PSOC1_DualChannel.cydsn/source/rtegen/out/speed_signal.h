...
*  file    SPEED_signal.h
*
*  brief   signal base class for signal speed
*
...
#ifndef SPEED_SIGNAL_H_INCLUDED
#define SPEED_SIGNAL_H_INCLUDED
 ...
 *  I-Port
 */
typedef RC_t (*SPEED_driverIn_t)(SPEED_data_t * const);

/**
 *  Definition of function types for driver interfaces.
 *  O-Port
 */
typedef RC_t (*SPEED_driverOut_t)( const SPEED_data_t * const);
...
/**
 * Instances of the struct SPEED_cfg_t represent the signal object configuration.
 */
struct SPEED_cfg_s
{
    /**
     ...
     */
    SPEED_driverIn_t const inDriver;

    /**
     * Function pointer to the output driver table
     */
   SPEED_driverOut_t const outDriver;
...
typedef const struct SPEED_cfg_s SPEED_cfg_t;
...
/**
 * Instances of the struct SPEED_t represent the signal objects.
...
{                                                                                                                      
     ...
     */
	const SPEED_cfg_t * const    pSigCfg;
 ...
 *
 * @param  SPEED_t * const signal: Pointer to the signal object which will be initialised
 * @param const speed_data_t * const value: Pointer to initial value of the application data
 * @return RC_t: standard return code, currently only RC_SUCCESS
 *
 **/
inline RC_t RTE_SPEED_init( SPEED_t * const signal, const SPEED_data_t * const value )
{                                                                                                                      
 ...
 *
 * @param  SPEED_t * const signal: Pointer to the signal object
 * @return  sig_status_t: The current signal status
 *
 **/
inline RTE_signalStatus_t RTE_SPEED_getStatus( const SPEED_t * const signal )
{
 ...
 *
 */
inline RC_t RTE_SPEED_setStatus( SPEED_t * const signal, const RTE_signalStatus_t status )
{
 ...
 */
inline uint32_t RTE_SPEED_getAge( const SPEED_t * const signal )
{
   return signal->age;
}
 ...
 * @return void
 */
inline void RTE_SPEED_incAge(  SPEED_t * const signal , uint32_t increment)
{
	   //Hack....
 ...
 *
 * @param  SPEED_t * const signal: Pointer to the signal object
 * @return speed_data_t: current value of the application data buffer
 *
 **/
inline SPEED_data_t RTE_SPEED_get( const SPEED_t * const signal )
{                                                                                                                      
   return signal->value;                                                                                               
}                                                                                                                      
 ...
 *
 * @param  SPEED_t * const signal: Pointer to the signal object
 * @param  speed_data_t const value: value for the application data buffer
 * @return RC_t: standard return code, currently only RC_SUCCESS
 *
 **/
inline RC_t RTE_SPEED_set(SPEED_t * const signal, const SPEED_data_t value  )
{
 ...
 *
 */
inline RC_t RTE_SPEED_pullPort( SPEED_t * const signal )
{                                                                                                                      
   RC_t ret = RC_SUCCESS;
   ...
   {
	  SPEED_data_t value = signal->value;
      ret = signal->pSigCfg->inDriver(&value);
                                                                                                                       
      if ( ret == RC_SUCCESS )                                                                                         
      {
    	  //Critical Section
    	  RTE_SPEED_set(signal, value);
      }
      else
      {
    	  //Set the status and fire error event if available
    	  RTE_SPEED_setStatus( signal, RTE_SIGNALSTATUS_INVALID );
 ...
 *
 **/
inline RC_t RTE_SPEED_pushPort( const SPEED_t * const signal )
{                                                                                                                      
   RC_t ret = RC_ERROR;
