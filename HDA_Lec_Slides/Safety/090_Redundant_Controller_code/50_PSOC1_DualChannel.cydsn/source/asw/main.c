...
TASK(tsk_Init)
{
...
#if WDG == WDG_INTERN
    WD_Start(WD_TO_1024);
    WD_ActivateCheckpoints(2);
#elif WDG == EDG_EXTERN_ALIVE
    WD_ActivateCheckpoints(2);
    EXWDG_Init(EXWDG_Alive);
#endif
    ...
    //Init Application runnables and CDD
    PEDAL_init_run();
    ENGINE_init_run();
    HMI_init_run();
    COM_init_run();
    
    //Set saftey Pin on upon startup
    ERROR_setSafteyPin();
    
    //Check for Watchdog reset
#if WDG_BOOTCHECK == ACTIVE
    if (TRUE == WD_CheckResetBit())
    {
        ERROR_centralHandler(ERROR_SYSTEM);
        //To avoid boot loop
        while(1)
        {
            WD_Trigger();
...
TASK(tsk_EventDispatcher)
{
    ...
    while(1)
    {
        ...
        //Trigger watchdog bit for event runnables
        WD_hitCheckpoint(0);
...
TASK(tsk_CyclicDispatcher)
{
    ...
    //Trigger watchdog bit for cyclic runnables
    WD_hitCheckpoint(1);
...
TASK(tsk_Background)
{
    while(1)
    {
        ...
        //Brute Force Watchdog Trigger
        #if WDG == WDG_INTERN
            if (TRUE == WD_allCheckpointsHit())
            {
                WD_Trigger();
            }
        #elif WDG == EDG_EXTERN_ALIVE
...
ISR2(isr_uart_psoc_rx)
{
    uint16_t error_data = UART_PSOC_GetByte();
    ...
    if (UART_PSOC_RX_STS_PAR_ERROR & error || 
        UART_PSOC_RX_STS_STOP_ERROR & error ||
        UART_PSOC_RX_STS_OVERRUN & error)
    {
        return;
    }
    
    //Synchronise on first run
    if (FALSE == UARTPROT_SyncProtocol(data))
    {
        //ignore data
        return;
    }
    ...
    if (UARTPROT_EOP1 == prevChar && UARTPROT_EOP2 == data)
    {
        //Complete protocol
        SetEvent(tsk_EventDispatcher, ev_uartIsr_onUpdate);
        prevChar = 0;
