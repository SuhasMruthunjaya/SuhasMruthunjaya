...
static float32_t ENG__integralError = 0;
static float32_t ENG__lastError = 0;
...
RC_t ENG_SetEngineClosedLoopPID(ENG_id_t id, uint16_t ticktime, sint16_t targetspeed, sint16_t measuredspeed, sint16_t* controlspeed)
{
    ...
    //Useless code to avoid MISRA warning
    id = id;
    
    //Comfort check: Avoid low speed beeping
    if (abs(targetspeed) < ENG_MINRPM)
    {
        targetspeed = 0;
    }
    
    //Control logic, all calculations using float32_t
    error = targetspeed - measuredspeed;
    ENG__integralError += (error * ticktime) / 1000;       
    differentialError = (error - ENG__lastError) * (1000/ticktime);
    ENG__lastError = error;

    //check I and D (TODO - not used at the moment) error for overrun
    if (ENG_CONTROL_KI * ENG__integralError > ENG_PWM_MAXDUTY) ENG__integralError = ENG_PWM_MAXDUTY/ENG_CONTROL_KI;
    if (ENG_CONTROL_KI * ENG__integralError < -ENG_PWM_MAXDUTY) ENG__integralError = -ENG_PWM_MAXDUTY/ENG_CONTROL_KI;
    
    float calSpeed = 0;
    calSpeed = (ENG_CONTROL_KP * error + ENG_CONTROL_KI * ENG__integralError + ENG_CONTROL_KD * differentialError);
    ...
    if (calSpeed > ENG_PWM_MAXDUTY) calSpeed = ENG_PWM_MAXDUTY;
    if (calSpeed < -ENG_PWM_MAXDUTY) calSpeed = -ENG_PWM_MAXDUTY;
    if (abs((sint16_t)calSpeed) < ENG_PWMMIN) calSpeed = 0;
    
    *controlspeed = (sint16_t)calSpeed;
...
/**
 * CHeck the palusibility of current speed and control speed
 * \param sint16_t measuredSpeed      : IN - The current measured speed [RPM]
 * \param sint16_t controlSpeed       : IN - The current controlspeed [PWM]
 * \return RC_SUCCESS
 */
RC_t ENG_checkControlspeedPlausibility(sint16_t measuredSpeed, sint16_t controlSpeed)
{
    //Search for the reference controlspeed form the open loop table
    //As we only look for a rough plausibility, we will only check the forward table
    //Values are similar enough for this
    measuredSpeed = abs(measuredSpeed);
    controlSpeed = abs(controlSpeed);
    sint16_t refControlSpeed = 0;
    for (uint16_t i = 0; i < ENG_cal_forward_size-1; i++)
    {
        if (measuredSpeed >= ENG_cal_forward[i].m_targetspeed && measuredSpeed < ENG_cal_forward[i+1].m_targetspeed)
        {
            refControlSpeed = (ENG_cal_forward[i].m_controlspeed + ENG_cal_forward[i+1].m_controlspeed) / 2;
            //Rough approximation
            break;
        }
    }
    
    LOG_I("ENG_SpeedPlausibility","C : %d R: %d", controlSpeed, refControlSpeed);
    
    //We mainly want to detect blocking situation
    if (controlSpeed - refControlSpeed > ENG_PWM_MAXDELTA) return RC_ERROR_INVALID_STATE; else return RC_SUCCESS;
    
}

/**
 * Reset the internal controller
 * \param ENG_id_t id       : IN - The engine identifier (use ENG_ALL) for all engines simultaniously
 * \return RC_SUCCESS
 */
RC_t ENG_ResetPID(ENG_id_t id)
{
    ENG__integralError = 0;
    ENG__lastError = 0;
}
