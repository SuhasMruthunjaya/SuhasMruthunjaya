...
/**
 * Will reset all signals
 */
static void CONTROL__reset_Car();
...
/*    Event                 ToState                 		 Guard                       Action                       */
    { EVENT(EV_IN_Reset),   STATE(CONNECTIONSTATE_IS_IDLE),      0,                          CONTROL__reset_Car},
 ...
 * shortname: control
 * signalIN: so_joystick|so_ultrasonic
...
static void CONTROL__reset_Car()
{
    ...
    RTE_reset_sp_common();
    //LOG_I("CONTROL__reset_Car","reset");
    
    //DET_report(&DET, DET_FSM_RESET, (uint32_t)STATE_getCurrentState(&CONTROL_ConnectionFSM));
}
