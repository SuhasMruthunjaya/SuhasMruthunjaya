...
/* signal configuration */
/*
 * description: Signal containing current position of car. Derived from so_decoderDelta
 * indriver: 0
 * name: so_position
 * onDataError: 0
 * onDataUpdate: 0
 ...
 * resource: None
 * shortname: position
 * signalclass: sc_position
 * signalpool: sp_common
 */

 

static const SC_POSITION_cfg_t SO_POSITION_cfg = 
{
   /* indriver           */  0,
   /* outdriver          */  0,
...
}; /* configuration so_position */


/* signal data */

SC_POSITION_t SO_POSITION_signal= 
{
   /* init value    */  SC_POSITION_INIT_DATA,
   /* status        */  RTE_SIGNALSTATUS_STARTUP,
   /* age           */  0,
   /* cfg           */  &SO_POSITION_cfg,
}; /* data so_position */
...
/* signal configuration */
/*
 * description: Signal containing decoder delta. Must be updated together with QuadDec call.
 * indriver: default
 * name: so_decoderDelta
 * onDataError: 0
 * onDataUpdate: ev_decoderdelta_onData
 * outdriver: 0
 * resource: None
 * shortname: decoderDelta
 * signalclass: sc_decoderDelta
 * signalpool: sp_common
 */


/* The following array contains the receiving tasks for onUpdate signal events */
const TaskTypeArray so_decoderDelta_updTasks = { tsk_control };
 

static const SC_DECODERDELTA_cfg_t SO_DECODERDELTA_cfg = 
{
   /* indriver           */  SC_DECODERDELTA_driverIn,
   /* outdriver          */  0,

   /* onUpdate           */  ev_decoderdelta_onData,
   /* onError            */  0,
   /* taskCount onUpdate */  1,
   /* task list onUpdate */  &so_decoderDelta_updTasks,
   /* taskCount onError  */  0,
   /* task list onError  */  NULL,
...
}; /* configuration so_decoderdelta */


/* signal data */

SC_DECODERDELTA_t SO_DECODERDELTA_signal= 
{
   /* init value    */  SC_DECODERDELTA_INIT_DATA,
   /* status        */  RTE_SIGNALSTATUS_STARTUP,
   /* age           */  0,
   /* cfg           */  &SO_DECODERDELTA_cfg,
}; /* data so_decoderdelta */
...
void RTE_timertick_sp_common_tick(uint32_t tick){
   ...
   RTE_SC_POSITION_incAge( &SO_POSITION_signal, tick); 
   RTE_SC_DECODERDELTA_incAge( &SO_DECODERDELTA_signal, tick); 
...
void RTE_reset_sp_common(){
   ...
   RTE_SC_CARSPEED_init( &SO_TARGETSPEED_signal); 
   RTE_SC_ENGINESPEED_init( &SO_CURRENTSPEED_signal); 
   RTE_SC_ENGINESPEED_init( &SO_CONTROLSPEED_signal); 
   RTE_SC_PROTOCOL_init( &SO_PROTOCOLRX_signal); 
   RTE_SC_PROTOCOL_init( &SO_PROTOCOLTX_signal); 
   RTE_SC_CARSTATE_init( &SO_CARSTATE_signal); 
   RTE_SC_EVENT_IN_init( &SO_EVENTIN_signal); 
   RTE_SC_EVENT_OUT_init( &SO_EVENTOUT_signal); 
   RTE_SC_CARERRORSTATE_init( &SO_CARERRORSTATE_signal); 
   RTE_SC_ULTRASONICARRAY_init( &SO_ULTRASONIC_signal); 
   RTE_SC_DIAGNOSTICREQUEST_init( &SO_DIAGNOSTICREQUEST_signal); 
   RTE_SC_POSITION_init( &SO_POSITION_signal); 
   RTE_SC_DECODERDELTA_init( &SO_DECODERDELTA_signal); 
