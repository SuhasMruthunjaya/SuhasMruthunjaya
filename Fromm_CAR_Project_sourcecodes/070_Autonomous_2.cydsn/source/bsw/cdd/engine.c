 ...
 * This function requires a cyclic call of ENG_updateDecoder
 * \param   ENG_t const* me                 : [IN] the engine object
 * \param   sint16_t *const decoderdelta    : [OUT] the decoder delta in ticks
 * \return RC_SUCCESS or error message
 */
RC_t ENG_getDecoder(ENG_t const * const me, sint16_t *const decoderdelta)
{
    *decoderdelta = me->m_deltaDecoder;
    return RC_SUCCESS;
}
...
RC_t ENG_updateDecoder(ENG_t * const me, uint16_t ticktime)
{
    ...
    if (RC_SUCCESS == result)
    {
        ...
        me->m_deltaDecoder = decoder;   //Decoder is set to 0 in cdd read function
    }
    else
    {
