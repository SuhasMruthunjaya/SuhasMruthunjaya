...
#if (Number_of_ISRs > 0) && defined Xbee_rx_isr__INTC_NUMBER
    ramVectorTable[CY_INT_IRQ_BASE + (Xbee_rx_isr__INTC_NUMBER & CY_INT_NUMBER_MASK)] = Xbee_rx_isr;
	//PF v2.5.3
    //NVIC_SET_PRI(Xbee_rx_isr__INTC_NUMBER, (ISR_MIN_PRI - 3));
    CyIntSetPriority(Xbee_rx_isr__INTC_NUMBER, ISR_SCALE(3));
    //\PF
	NVIC_INT_ENABLE(Xbee_rx_isr__INTC_NUMBER);
#elif Number_of_ISRs > 0
#error  The Interrupt Xbee_rx_isr could not be found. Make sure the name in the OS config exactly matches the name of the interrupt in the interrupt tab.
#endif	/* Xbee_rx_isr */

#if (Number_of_ISRs > 1) && defined isr_US_Timer_Stop__INTC_NUMBER
    ramVectorTable[CY_INT_IRQ_BASE + (isr_US_Timer_Stop__INTC_NUMBER & CY_INT_NUMBER_MASK)] = isr_US_Timer_Stop;
	//PF v2.5.3
    //NVIC_SET_PRI(isr_US_Timer_Stop__INTC_NUMBER, (ISR_MIN_PRI - 4));
    CyIntSetPriority(isr_US_Timer_Stop__INTC_NUMBER, ISR_SCALE(4));
    //\PF
	NVIC_INT_ENABLE(isr_US_Timer_Stop__INTC_NUMBER);
#elif Number_of_ISRs > 1
#error  The Interrupt isr_US_Timer_Stop could not be found. Make sure the name in the OS config exactly matches the name of the interrupt in the interrupt tab.
#endif	/* isr_US_Timer_Stop */

#if (Number_of_ISRs > 2) && defined isr_US_Timer_Start__INTC_NUMBER
    ramVectorTable[CY_INT_IRQ_BASE + (isr_US_Timer_Start__INTC_NUMBER & CY_INT_NUMBER_MASK)] = isr_US_Timer_Start;
	//PF v2.5.3
    //NVIC_SET_PRI(isr_US_Timer_Start__INTC_NUMBER, (ISR_MIN_PRI - 4));
    CyIntSetPriority(isr_US_Timer_Start__INTC_NUMBER, ISR_SCALE(4));
    //\PF
	NVIC_INT_ENABLE(isr_US_Timer_Start__INTC_NUMBER);
#elif Number_of_ISRs > 2
#error  The Interrupt isr_US_Timer_Start could not be found. Make sure the name in the OS config exactly matches the name of the interrupt in the interrupt tab.
#endif	/* isr_US_Timer_Start */

#if (Number_of_ISRs > 3) && defined isr_sw_tick__INTC_NUMBER
    ramVectorTable[CY_INT_IRQ_BASE + (isr_sw_tick__INTC_NUMBER & CY_INT_NUMBER_MASK)] = isr_sw_tick;
	//PF v2.5.3
    //NVIC_SET_PRI(isr_sw_tick__INTC_NUMBER, (ISR_MIN_PRI - 5));
    CyIntSetPriority(isr_sw_tick__INTC_NUMBER, ISR_SCALE(5));
    //\PF
...
#if (Number_of_ISRs > 4) && defined isr_uart_usb__INTC_NUMBER
    ramVectorTable[CY_INT_IRQ_BASE + (isr_uart_usb__INTC_NUMBER & CY_INT_NUMBER_MASK)] = isr_uart_usb;
	//PF v2.5.3
    //NVIC_SET_PRI(isr_uart_usb__INTC_NUMBER, (ISR_MIN_PRI - 4));
    CyIntSetPriority(isr_uart_usb__INTC_NUMBER, ISR_SCALE(4));
    //\PF
	NVIC_INT_ENABLE(isr_uart_usb__INTC_NUMBER);
#elif Number_of_ISRs > 4
#error  The Interrupt isr_uart_usb could not be found. Make sure the name in the OS config exactly matches the name of the interrupt in the interrupt tab.
#endif	/* isr_uart_usb */
