...
const RTE_cyclicTable_t RTE_cyclicActivationTable_tsk_io[] = {
	...
	{ MONITOR_health_run, 10 },	//Cyclic runnable checking the system health
	{ SENSOR_update_run, 25 },	//Runnable updating the next ultrasonic sensor of the arrays.
};
...
const RTE_eventTable_t RTE_eventActivationTable_tsk_io[] = {
    ...
    { MONITOR_processDiagnosticRequest_run, ev_diagnosticrequest_onData },  //Receive Diagnostic Request and processes it, e.g clears DETor transmits DET to client
}; 
...
TASK(tsk_io)
{
    ...
    while(1)
    {
        ...
        WaitEvent(ev_diagnosticrequest_onData|ev_eventout_onData|ev_protocolrx_onError|ev_tick_1ms|ev_xbee_rx);
        GetEvent(tsk_io,&ev);
        ClearEvent(ev);
...
//Will be called every 100us for the Ultrasonic sensor
ISR(isr_sw_tick)
{
    //Will be called every 100us
    US_TickTimer(100);
}

ISR(isr_US_Timer_Start)
{
    //Start the software clock
    US_ResetTimer();
}

ISR(isr_US_Timer_Stop)
{
    
    //Store the time - should happen at the beginning
    uint32_t timeOfFlight = US_GetTimer();
    
    //Read in signal to be updated
    SC_ULTRASONICARRAY_data_t data = RTE_SC_ULTRASONICARRAY_get(&SO_ULTRASONIC_signal);
    
    switch (data.m_index)
    {
        case 0 : US_SetDistance_isr(&US_front, timeOfFlight); break;
        case 1 : US_SetDistance_isr(&US_rear, timeOfFlight); break;
        case 2 : US_SetDistance_isr(&US_left, timeOfFlight); break;
        case 3 : US_SetDistance_isr(&US_right, timeOfFlight); break;
        default: break;
    }
}
...
ISR(isr_uart_usb)
{
    ...
    SC_DIAGNOSTICREQUEST_data_t command = SC_DIAGNOSTICREQUEST_INIT_DATA;
    ...
    {
        case 'r' :
        case 'R' : command = DET_RequestData; break;
        case 'c' :
        case 'C' : command = DET_ClearData; break;
        default: break;
    }
    
    if (SC_DIAGNOSTICREQUEST_INIT_DATA != command)
    {
        //valid data found
        RTE_SC_DIAGNOSTICREQUEST_set(&SO_DIAGNOSTICREQUEST_signal, command);
    }
